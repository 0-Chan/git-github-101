import type { ParsedCommand, ShellContext, CommandResult } from '@/types'
...
export async function executeCommand(parsed: ParsedCommand, ctx: ShellContext): Promise<CommandResult> {
  if (parsed.command === 'git') {
    ...
    if (!handler) return { output: `git: '${subcommand}' is not a git command.\n💡 '도움말'을 입력하면 사용 가능한 명령어를 볼 수 있어요.`, isError: true }
  ...
  return { output: `이 명령어는 아직 지원하지 않아요.\n💡 '도움말'을 입력하면 사용 가능한 명령어를 볼 수 있어요.`, isError: true }
}
