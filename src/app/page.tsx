          ...
          {sections.map((section) => (
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                <span className="font-mono text-sm text-zinc-400">{String(section.order).padStart(2, "0")}</span>
                <h3 className="font-semibold">{section.title}</h3>
                {!section.hasTerminal && (
                  <span className="text-xs px-2 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded text-zinc-500">읽기</span>
                )}
              </div>
