...
>
> **2026-07-06 축소 결정: 리더보드·체크인/체크아웃·자가진단 UI를 제거했다** (불필요 판단).
> 남은 것: 회차 페이지(시간표·강의·레슨 링크·미션 체크)와 로컬 이벤트 로그
> (`git101-events` — 레슨·미션 진행 표시에 사용, ProgressEvent 스키마는 checkin/survey
> kind 포함 그대로 유지). 아래 리더보드·체크인·Supabase 절은 재도입할 때의 참고용
> 설계 기록으로 남긴다.
