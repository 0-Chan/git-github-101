...
describe('git commands', () => {
  ...
  describe('git log', () => {
    it('shows commit history with message', async () => {
      ...
      await git.commit({ fs, dir: '/', message: 'first commit', author: { name: '학습자', email: 'learner@git101.dev' } })
  ...
  describe('git merge', () => {
    ...
    it('with conflict — sets pendingMerge, output contains CONFLICT', async () => {
      ...
      await git.add({ fs, dir: '/', filepath: 'file.txt' })
      await git.commit({ fs, dir: '/', message: 'feature change', author: { name: '학습자', email: 'learner@git101.dev' } })
