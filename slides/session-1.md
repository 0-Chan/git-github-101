...
3일차와 4일차는 심화 과정입니다. 3일차에는 GitHub Actions로 CI/CD 파이프라인을 직접 구축하고, 코드 리뷰를 주고받는 법까지 익힙니다. 커밋할 때마다 자동 검사가 돌고 동료가 리뷰해주는, 실무 그대로의 흐름이죠. 4일차에는 실제 오픈소스 프로젝트에 기여하는 것에 도전하고, 여러분의 GitHub 저장소와 프로필을 포트폴리오로 완성합니다. 수업이 끝나면 채용담당자에게 보여줄 수 있는 저장소가 실물로 남습니다.
