...
<div class="grid grid-cols-3 gap-4 pt-6">
  ...
  <div class="rounded-lg border border-sky-300/40 p-5">
...
학생들이 외워야 하는 것은 옵션 이름 자체보다 "내가 지금 보존하고 싶은 변경이 어디에 있는가"입니다. 작업 디렉터리에 남겨야 할 변경이 있다면 --hard를 쓰면 안 됩니다.
-->

---

# `restore`는 파일 단위 되돌리기에 좋습니다
...
<div class="grid grid-cols-3 gap-4 pt-6">
  <div class="rounded-lg border border-emerald-300/40 p-4">
...
# 고급 명령어는 상황 이름으로 기억합니다

<div class="grid grid-cols-2 gap-4 pt-4 text-xl">
  <div class="rounded-lg border border-sky-300/40 p-4"><code>log/show/diff</code><br><span class="opacity-70">먼저 읽을 때</span></div>
  <div class="rounded-lg border border-emerald-300/40 p-4"><code>switch</code><br><span class="opacity-70">브랜치를 바꿀 때</span></div>
  <div class="rounded-lg border border-amber-300/40 p-4"><code>cherry-pick</code><br><span class="opacity-70">커밋 하나만 가져올 때</span></div>
  <div class="rounded-lg border border-violet-300/40 p-4"><code>bisect</code> / <code>blame</code><br><span class="opacity-70">원인을 좁힐 때</span></div>
</div>
...
<div class="text-3xl font-mono pt-8 leading-relaxed">
...
학생들이 "왜 자동 검사가 필요한가"를 납득한 뒤 GitHub Actions 문법으로 넘어가게 합니다.
-->

---

# CI/CD는 도구 이름이 아니라 습관입니다
...
<!--
CI/CD를 처음부터 GitHub Actions YAML로 설명하면 학생들은 "파일 하나 외우는 일"로 받아들이기 쉽습니다.
...
그래서 반복 검사를 PR에 자동 피드백으로 붙이는 것이 CI의 출발점입니다.
-->
...
PR 검사에서 실패하면 바로 고치고 다시 push합니다. 이 짧은 반복이 피드백 루프입니다.
-->

---

# CD는 배포 가능한 상태를 유지합니다
...
용어 설명에 오래 머물기보다 작은 변경, 빠른 피드백, 반복 개선이라는 실무 감각을 잡아줍니다.
-->
...
<div class="grid grid-cols-2 gap-6 pt-5">
  <div class="rounded-lg border border-rose-300/40 p-5">
    <div class="font-bold text-xl pb-3">큰 배치</div>
    <div class="font-mono text-lg opacity-80">일주일 작업 -> 큰 PR -> 큰 충돌</div>
  </div>
  <div class="rounded-lg border border-emerald-300/40 p-5">
    <div class="font-bold text-xl pb-3">작은 배치</div>
    <div class="font-mono text-lg opacity-80">작은 PR -> 빠른 검사 -> 빠른 merge</div>
  </div>
</div>
...
학생들이 가져갈 메시지는 XP 이름 자체보다 작은 변경, 자동 테스트, 빠른 피드백입니다.
-->

---

# 자동화는 팀의 기준을 같은 곳에 둡니다
...
이후 슬라이드에서는 문법을 많이 나열하기보다, 워크플로 파일 하나를 읽을 수 있게 만드는 것을 목표로 합니다.
-->

---

# Actions는 저장소 안의 워크플로를 실행합니다
...
오늘은 실제 파일을 추가하지 않고 덱 안에서 예제로만 다룹니다.
-->

---

# 워크플로는 “언제, 어디서, 무엇을”입니다
...
# `actions/checkout`은 코드를 runner에 내려받습니다
...
<div class="opacity-75 pb-8 -mt-1">저장소에 테스트 또는 빌드 확인을 붙이고 PR에서 확인하기</div>
...
실제 예제 저장소 기준으로 어떤 명령을 CI에 걸지 확인하면서 진행합니다.
-->

---

# 실습 목표는 PR Checks를 만드는 것입니다
...
<div class="pt-6 opacity-75">
오늘의 성공 기준은 “merge 전에 결과를 볼 수 있다”입니다.
</div>
...
즉 ci.yml 작성은 수단이고, PR 피드백이 결과입니다.
-->
...
처음 실습에서는 pull_request만으로 시작합니다. PR을 만들거나 업데이트했을 때 자동 검사가 돌기 때문에, 학생들이 결과를 PR 화면에서 바로 확인할 수 있습니다.
...
이번 덱 자체는 Slidev 빌드로 검증하지만, 수업 예제는 더 일반적인 Node 저장소 기준으로 보여줍니다.
-->
...
학생들이 구조를 읽을 수 있으면 그 다음에 lint나 test를 추가하면 됩니다.
-->

---

# 5단계: PR의 Checks를 읽습니다
...
# CD는 Git 흐름의 끝이 아니라 다음 루프입니다

<div class="text-3xl font-mono pt-8 leading-relaxed">
merge -> tag -> release -> observe
</div>
...
# worktree는 한 저장소를 여러 작업 공간으로 엽니다
...
<div class="text-3xl font-mono pt-8 leading-relaxed">
branch -> CI -> review -> release -> experiment
</div>
