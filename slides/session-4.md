...
다섯 가지 실습을 명령어·기대 결과·흔한 오류 순서로 보겠습니다.
-->
...
# 실습 ④: Issue부터 Draft PR까지

```bash
# 에이전트에게: "Issue 12번 읽고 요구사항 분석해서 브랜치 만들고 구현해줘.
#              테스트 통과하면 커밋하고 Draft PR 만들어줘"
gh issue view 12          # 요구사항 읽기
gh issue develop 12       # Issue에 연결된 브랜치 생성
# ... 구현하고 테스트 ...
git add -A && git commit -m "..." && git push
gh pr create --draft      # 완성 전이라 초안으로
```

<div class="grid grid-cols-2 gap-4 pt-4">
  <div class="s3-card s3-card--emerald p-4">
    <div class="font-bold pb-1">기대 결과</div>
    <div class="text-sm opacity-80">Issue에 연결된 브랜치가 생기고, 구현이 Draft PR로 올라갑니다.</div>
  </div>
  <div class="s3-card s3-card--rose p-4">
    <div class="font-bold pb-1">흔한 오류</div>
    <div class="text-sm opacity-80">테스트 실패 시 커밋이 멈춤, Issue 번호·저장소 권한 확인</div>
  </div>
</div>

<div class="pt-4 text-center opacity-75">Claude Code와 Git·GitHub를 함께 쓰는 모습, 그 자체입니다.</div>

<!--
이 실습이 오늘 gh 파트의 핵심입니다. Issue 하나를 골라 읽는 것부터 Draft PR을 올리는 것까지, 사람이 명령을 일일이 치지 않고 자연어 한 문단으로 맡깁니다.

에이전트는 안에서 gh issue view로 요구사항을 읽고, gh issue develop으로 그 Issue에 연결된 브랜치를 만든 뒤, 구현하고 테스트가 통과하면 커밋해서 gh pr create --draft로 초안 PR을 올립니다. gh는 이렇게 Issue 조회·생성·수정, Issue와 연결된 브랜치 생성, PR 생성·검토·병합까지 지원합니다.

Draft로 여는 이유는 아직 완성 전이기 때문입니다. CI와 리뷰를 먼저 돌려보고, 준비되면 리뷰 요청으로 바꿉니다. 이 흐름이 여러분이 기대한 "Claude Code와 Git·GitHub를 함께 쓰는 모습"에 가장 가깝습니다.
-->
...
```bash
# 에이전트에게: "현재 브랜치와 main의 차이를 분석해서,
#              목적·구현·테스트 방법·주의 사항을 담은 PR 설명 쓰고 PR 만들어줘"
git diff main...HEAD      # 무엇이 바뀌었는지 분석
gh pr create              # 분석한 내용으로 채운 설명과 함께
```

<div class="grid grid-cols-2 gap-4 pt-5">
  <div class="s3-card s3-card--emerald p-4">
    <div class="font-bold pb-1">기대 결과</div>
    <div class="text-sm opacity-80">diff에 근거해 목적·구현·테스트·주의가 채워진 PR 설명</div>
  </div>
  <div class="s3-card s3-card--rose p-4">
    <div class="font-bold pb-1">흔한 오류</div>
    <div class="text-sm opacity-80">변경이 너무 크면 요약이 얕아짐, 범위를 좁혀 요청</div>
  </div>
</div>

<!--
PR 설명을 매번 손으로 쓰는 건 번거롭습니다. 에이전트에게 현재 브랜치와 main의 차이를 분석하게 하면, 실제 변경 내용에 근거해 설명을 채워줍니다.

2일차에서 PR은 "무엇을, 왜, 확인 방법"을 담는 문서라고 했습니다. 여기서는 그 문서를 diff를 기준으로 자동으로 초안 잡는 셈입니다. 목적, 주요 구현, 테스트 방법, 주의 사항을 항목으로 요청하면 리뷰어가 읽기 좋은 형태로 나옵니다.

주의할 점은 변경 범위입니다. 한 PR에 너무 많은 게 섞이면 요약도 얕아집니다. 좋은 커밋 단위와 작은 PR이 좋은 설명으로 이어집니다.
-->
