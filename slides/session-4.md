...
<!--
4회차를 시작하겠습니다. 오늘은 실제 오픈소스 프로젝트에 기여하는 것에 도전하고, 여러분의 GitHub 저장소와 프로필을 포트폴리오로 완성합니다. 수업이 끝나면 보여줄 수 있는 실물이 남는 날이에요.
-->
...
<!--
오늘은 먼저 worktree로 작업 공간을 나누는 법부터 익힙니다.

그다음 이슈를 고르고, 별도 작업 공간에서 수정하고, PR로 기여하는 흐름을 연결합니다. 마지막에는 저장소와 GitHub 프로필을 포트폴리오 관점에서 다듬습니다.
-->
...
한 폴더에서 main과 feature 브랜치를 오가려면 수정 중인 파일을 먼저 커밋하거나 stash해야 할 수 있습니다. 실행 중인 개발 서버도 같은 폴더를 바라봅니다.
...
<!--
브랜치와 worktree를 같은 것으로 이해하면 명령이 어려워집니다.

브랜치는 커밋 이력에서 갈라진 포인터입니다. worktree는 그 브랜치의 파일을 실제로 펼쳐 놓은 작업 폴더입니다.

하나의 Git 저장소 데이터를 공유하지만, 폴더마다 현재 브랜치와 수정 중인 파일은 따로 관리됩니다.
-->
...
<!--
처음에는 평소처럼 저장소를 clone한 폴더 하나가 있습니다. 이 폴더에서는 main 브랜치를 열고 있다고 가정합니다.

Git 저장소 데이터와 작업 폴더가 하나씩 연결된 익숙한 상태입니다.

이제 main 폴더를 그대로 둔 채, feature/profile 브랜치를 위한 두 번째 폴더를 옆에 만들겠습니다.
-->
...
<!--
이 명령 하나가 새 브랜치와 새 작업 폴더를 함께 만듭니다.

add 뒤에는 새 폴더의 경로가 필요합니다. -b 뒤에는 새로 만들 브랜치 이름이 옵니다. 마지막 main은 새 브랜치를 어느 지점에서 시작할지 정합니다.
...
<!--
Git은 먼저 feature/profile 브랜치를 만들었다고 알려 줍니다. 이어서 새 폴더의 HEAD가 어떤 커밋을 가리키는지 보여 줍니다.
...
<!--
각 폴더에서 git branch --show-current를 실행해 보면 서로 다른 결과가 나옵니다.

원래 폴더는 계속 main을 보고 있고, 새 worktree는 feature/profile을 보고 있습니다.

에디터 창도 폴더별로 하나씩 열 수 있습니다. 이 상태에서는 main 화면과 새 기능 화면을 오가며 바로 비교할 수 있습니다.
-->
...
<div class="pt-5 text-lg opacity-80">
커밋 데이터는 공유하지만, 작업 파일과 staging area는 폴더마다 따로 관리합니다.
</div>

<!--
feature/profile 폴더에서 README를 수정했다고 가정합니다. 이 폴더의 status에는 수정 파일이 나타납니다.

main 폴더로 돌아가 같은 명령을 실행하면 아무것도 나오지 않습니다. main의 작업 파일은 바뀌지 않았기 때문입니다.

각 worktree는 작업 디렉터리와 staging area를 따로 가지므로, 한쪽의 미완성 변경이 다른 쪽의 작업 화면을 어지럽히지 않습니다.
-->
...
즉 worktree는 협업 절차를 바꾸는 기능이 아니라, 그 절차를 수행할 로컬 작업 공간을 나누는 기능입니다.
-->
...
수정 파일이 남아 있으면 Git이 제거를 막습니다. 먼저 <code>git status</code>로 확인합니다.
</div>

<!--
작업이 끝나면 폴더를 Finder나 rm으로 바로 지우지 말고 git worktree remove를 사용합니다. 그래야 Git의 연결 정보와 폴더가 함께 정리됩니다.

먼저 pull --ff-only로 로컬 main에 병합 결과를 가져옵니다. 그다음 병합된 로컬 브랜치를 branch -d로 지웁니다. -d는 병합되지 않은 커밋이 있으면 삭제를 거부하므로 초보자에게 더 안전합니다.

마지막으로 worktree list를 다시 실행해 원래 main 폴더만 남았는지 확인합니다.
-->
...
<!--
첫째, Git은 같은 브랜치를 두 worktree에서 동시에 체크아웃하지 못하게 막습니다. list로 어느 폴더에서 사용 중인지 찾습니다.

둘째, 수정하거나 새로 만든 파일이 남아 있으면 remove가 실패합니다. --force부터 쓰지 말고 status로 보존할 작업인지 판단합니다.

셋째, node_modules와 빌드 결과처럼 Git이 추적하지 않는 파일은 새 폴더에 자동으로 생기지 않습니다. 필요한 설치를 다시 실행합니다.

넷째, 개발 서버를 두 개 켜면 같은 포트를 쓰려다 충돌할 수 있습니다. 비교 실행 시 포트를 나눕니다.
-->
...
<!--
이제 worktree를 4일차의 오픈소스 기여 흐름에 연결합니다.
...
<!--
worktree로 작업 공간을 나눴다면, 이제 그 작업을 GitHub에 올려 PR로 만들 차례입니다.
...
<!--
핵심은 사람이 gh 문법을 다 외우는 게 아닙니다. 하고 싶은 일을 자연어로 말하면, 에이전트가 적절한 gh 명령으로 옮겨 실행합니다.
...
새 PR을 또 만들 필요가 없다는 게 핵심입니다. 같은 브랜치에 push만 하면 됩니다.
-->
...
<!--
정리해 보겠습니다. gh는 GitHub 작업을 터미널로 가져오는 도구이고, 코딩 에이전트는 그 gh를 자연어로 구동하는 조종석입니다.
