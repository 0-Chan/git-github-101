...
작업 공간을 나누고, 터미널에서 PR 흐름까지 이어갑니다
</div>
...
그다음 gh CLI로 이슈와 PR, 자동 검사와 리뷰 흐름을 터미널에서 이어갑니다.

목차에 보이는 두 구간은 오늘 실제로 진행할 순서입니다. worktree로 작업 공간을 안전하게 나누고, gh로 그 결과를 GitHub 협업 흐름에 연결합니다.
-->
...
<div class="text-sm opacity-60 font-mono pb-2">OPEN SOURCE</div>

# 이제 AI와 함께 세상의 코드에 기여합니다

<div class="opacity-75 pb-6 -mt-1">메커니즘은 2·3회차에 다 배웠습니다. 이제 어디서·무엇을·어떤 예의로 시작하느냐입니다</div>

<div class="text-2xl leading-relaxed">
worktree로 나눈 작업 공간 + gh + <span style="color: var(--lane-main)" class="font-bold">AI</span><br>
= 첫 오픈소스 기여의 도구
</div>

<!--
2일차에 fork와 PR을, 3일차에 CI와 코드 리뷰를 배웠습니다. 남의 저장소에 기여하는 메커니즘은 이미 손에 있습니다.

그래서 이 섹션은 fork를 다시 설명하지 않습니다. 대신 남의 프로젝트에서 어떤 이슈를 고르고, 규칙을 어떻게 따르고, 어떤 예의로 시작하는지를 봅니다.

그리고 그 과정 전체를 AI로 가속합니다. AI는 이슈를 읽고 분석하는 시간을 크게 줄여 주는 지렛대입니다.
-->
...
<div class="text-lg opacity-75 -mt-2 pb-3">라벨과 재현법이 이슈의 "기여 난이도"를 알려 줍니다</div>

<div class="grid grid-cols-2 gap-4 items-stretch">
  <div class="flex flex-col gap-3">
    <div class="s3-card s3-card--emerald p-3">
      <div class="text-sm font-bold pb-1">고르면 좋은 신호</div>
      <div class="text-xs opacity-85 leading-snug">
        <div><code>good first issue</code> · <code>help wanted</code></div>
        <div>재현법·로그 명확 · 아직 PR 없음</div>
        <div>메인테이너 확인 · 최근 커밋 활발</div>
      </div>
    </div>
    <div class="s3-card s3-card--rose p-3">
      <div class="text-sm font-bold pb-1">피하면 좋은 신호</div>
      <div class="text-xs opacity-85 leading-snug">
        <div><code>blocked</code> · <code>wait-for-triage</code></div>
        <div>논의만 길고 방향 미정</div>
        <div>Jira 등 별도 트래커 사용</div>
      </div>
    </div>
  </div>
...
<div class="text-xs opacity-70 pt-1">탐색: GitHub Issues 라벨 필터 · goodfirstissue.dev · GitHub Trending</div>

<!--
라벨은 GitHub가 이슈에 붙이는 분류표입니다. good first issue와 help wanted는 메인테이너가 "초심자를 환영한다"고 손을 든 이슈입니다.

초심자는 라벨이 있고 재현 방법이 적혀 있으며 아직 아무도 PR을 안 올린 이슈부터 고르는 게 안전합니다. blocked나 wait-for-triage가 붙었으면 아직 손댈 때가 아닙니다.

프로젝트 자체는 최근 커밋과 메인테이너 응답이 활발한지 보고 고릅니다. goodfirstissue.dev나 GitHub Trending으로 시작점을 찾아도 좋습니다.
-->

---

# AI에게 이슈를 분석시키고 방향을 잡습니다

<div class="flex items-center justify-center gap-1.5 pt-3 text-sm">
  <div class="shrink-0 whitespace-nowrap rounded-lg border border-amber-300/40 px-3 py-3 text-center">이슈 후보 수집</div>
  <div class="opacity-50">-></div>
  <div class="shrink-0 whitespace-nowrap rounded-lg border border-violet-300/40 px-3 py-3 text-center">AI로 난이도 분류</div>
  <div class="opacity-50">-></div>
  <div class="shrink-0 whitespace-nowrap rounded-lg border border-sky-300/40 px-3 py-3 text-center">Claude Code로 파일 분석</div>
  <div class="opacity-50">-></div>
  <div class="shrink-0 whitespace-nowrap rounded-lg border border-emerald-300/40 px-3 py-3 text-center">수정 방향 확정</div>
</div>

<div class="text-sm font-bold pt-4 pb-1" style="color: var(--lane-main)">① 검색 가능한 AI에게 — 이슈 고르기</div>

```text
너는 오픈소스 기여 경험 많은 시니어야.
아래 저장소의 열린 이슈를 초심자가 기여하기 좋은 순서로
난이도(상·중·하)와 이유를 붙여 정리해줘.
저장소: github.com/OWNER/REPO
기준: good first issue · 재현법 명확 · 아직 PR 없음
```

<div class="text-sm font-bold pt-2 pb-1" style="color: var(--lane-main)">② 클론 후 Claude Code에게 — 해결 방향</div>

```text
이슈 #123을 해결하려면 어떤 파일을 고쳐야 할까?
관련 코드를 찾아 설명하고, 최소 변경 방안을 제안해줘.
아직 수정하지 말고 계획만.
```

<!--
AI가 오픈소스 기여의 진입장벽을 낮추는 지점이 바로 여기입니다. 예전엔 이슈를 읽고 원인을 파악하는 데만 몇 시간이 걸렸습니다.

먼저 검색 가능한 AI에게 저장소의 이슈들을 난이도별로 분류시켜 좋은 후보를 추립니다. 그다음 저장소를 클론하고 Claude Code에게 어떤 파일을 고쳐야 하는지 분석시킵니다.

"아직 수정하지 말고 계획만"이 중요합니다. 방향을 먼저 확인하고, 실제 코드는 규칙을 읽고 이슈에 말을 건 다음에 짭니다.

(참고: Medium 오픈소스 기여모임, "오픈소스의 판도를 바꿀 — AI로 오픈소스 기여 완벽 가이드와 프롬프트 공유"
https://medium.com/opensource-contributors/2db85bf736b8)
-->

---

# 손대기 전에 그 프로젝트의 규칙을 읽습니다

<div class="text-lg opacity-75 -mt-2 pb-3">프로젝트마다 기여 규칙이 다릅니다. 코드보다 규칙을 먼저 읽습니다</div>

<div class="grid grid-cols-2 gap-4">
  <div class="s3-card s3-card--sky p-4">
    <div class="font-mono text-sm font-bold pb-1">CONTRIBUTING.md</div>
    <div class="text-sm opacity-75">기여 방법 · PR 규칙 · 커밋 컨벤션</div>
  </div>
  <div class="s3-card s3-card--sky p-4">
    <div class="font-mono text-sm font-bold pb-1">README</div>
    <div class="text-sm opacity-75">빌드 · 셋업 · 테스트 실행법</div>
  </div>
  <div class="s3-card s3-card--sky p-4">
    <div class="font-mono text-sm font-bold pb-1">CODE_OF_CONDUCT.md</div>
    <div class="text-sm opacity-75">커뮤니티 행동 강령</div>
  </div>
  <div class="s3-card s3-card--sky p-4">
    <div class="font-mono text-sm font-bold pb-1">Issue · PR 템플릿</div>
    <div class="text-sm opacity-75">3회차에서 본 그 양식 — 채워 달라는 항목</div>
  </div>
</div>

<div class="pt-4 text-base opacity-80">규칙을 무시한 PR은 <span style="color: var(--lane-main)">리뷰가 시작되기도 전에</span> 닫히기도 합니다.</div>

<!--
어느 프로젝트든 처음 들어가면 CONTRIBUTING.md부터 찾습니다. 여기에 PR을 어떻게 올리고 커밋 메시지를 어떻게 쓰라는 규칙이 적혀 있습니다.

README는 빌드와 테스트 실행법을, CODE_OF_CONDUCT는 커뮤니티에서 지켜야 할 태도를 담습니다. 이슈와 PR 템플릿은 3회차에서 본 그 양식입니다.

이 규칙들을 무시하면 코드가 아무리 좋아도 리뷰가 시작되기 전에 닫힐 수 있습니다. 규칙을 먼저 읽는 게 첫 기여의 예의입니다.
-->
...
<div class="text-lg opacity-75 -mt-2 pb-3">"제가 해볼게요" 한마디가 중복 작업과 헛수고를 막습니다</div>

```bash
# 이슈에 의사 표시 — 중복 방지 · assignee 요청
gh issue comment 123 --body "제가 이 이슈를 맡아보고 싶습니다."
```

<div class="grid grid-cols-2 gap-4 pt-4">
  <div class="s3-card s3-card--emerald p-4">
    <div class="font-bold pb-1">작은 변경</div>
    <div class="text-sm opacity-80">오타·문서·명백한 버그는 바로 PR을 올려도 됩니다.</div>
  </div>
  <div class="s3-card s3-card--amber p-4">
    <div class="font-bold pb-1">크거나 복잡한 변경</div>
    <div class="text-sm opacity-80">방향을 이슈에서 먼저 합의한 뒤 코드를 시작합니다.</div>
  </div>
</div>

<!--
오픈소스는 비동기 협업입니다. 아무 말 없이 큰 PR을 던지면 이미 누가 작업 중이거나 방향이 다를 때 통째로 헛수고가 됩니다.

그래서 이슈에 "제가 맡아보고 싶습니다" 한 줄을 먼저 답니다. 중복을 막고, 원하면 담당자로 지정받을 수도 있습니다.

오타나 문서 수정처럼 작고 명백한 건 바로 PR을 올려도 됩니다. 하지만 설계가 얽힌 큰 변경은 방향을 이슈에서 먼저 합의한 뒤 코드를 시작하는 게 안전합니다.
-->

---

# 기여 루프는 이미 배운 것의 조합입니다

<div class="text-lg opacity-75 -mt-2 pb-2">새로 배울 건 없습니다 — worktree와 gh를 오픈소스에 그대로 얹습니다</div>

```mermaid {scale: 0.46, theme: 'base', themeVariables: {primaryColor: '#27272a', primaryTextColor: '#ffffff', primaryBorderColor: '#f5a524', lineColor: '#f5a524', fontSize: '15px'}}
flowchart LR
  U[(upstream · 원본)] -->|fork| O[origin · 내 저장소]
  O -->|worktree| L[로컬 작업 + 검사]
  L -->|push| O
  O -->|Pull Request| U
```

<div class="rounded-lg border border-gray-400/30 bg-black/30 p-3 text-sm mt-2">
  <div><code>gh repo sync MY_ID/REPO --branch main</code> — 원격 fork 동기화</div>
  <div class="pt-1"><code>git fetch upstream && git merge upstream/main</code> — 로컬 최신화</div>
  <div class="text-xs opacity-70 pt-1">둘 중 하나로 원본 변경을 반영한 뒤 테스트합니다.</div>
</div>

<!--
이 그림의 모든 화살표는 이미 배운 것입니다. 원본을 fork해 내 저장소를 만들고, worktree로 작업 폴더를 열고, 커밋해서 push하고, PR로 원본에 제안합니다.

2일차에 배운 origin과 upstream 두 remote가 여기서 다시 나옵니다. origin은 내 fork, upstream은 원본입니다.

유일하게 새로 챙길 습관은 fork 동기화입니다. PR을 올리기 전에 upstream의 최신 변경을 반영하고 테스트가 통과하는지 확인하면 리뷰가 깔끔해집니다.
-->
...
<div class="text-lg opacity-75 -mt-2 pb-3">이 덱은 AI를 계속 권했습니다. 그만큼 이 경계도 분명히 합니다</div>

<div class="grid grid-cols-2 gap-4">
  <div class="s3-card s3-card--emerald p-4">
    <div class="font-bold pb-2">이렇게</div>
    <div class="text-sm opacity-85 leading-relaxed">
      <div>PR 전에 방향·정확성을 직접 검증</div>
      <div>최소한의 변경 + 테스트 포함</div>
      <div><code>Fixes #123</code>와 "무엇을 왜" 설명</div>
    </div>
  </div>
  <div class="s3-card s3-card--rose p-4">
    <div class="font-bold pb-2">이건 지양</div>
    <div class="text-sm opacity-85 leading-relaxed">
      <div>AI 출력을 그대로 대량 PR</div>
      <div>검증 없이 그냥 제출</div>
      <div>논의 없이 큰 변경 · 메인테이너 시간 낭비</div>
    </div>
  </div>
</div>

<div class="pt-4 text-xl text-center font-bold"><span style="color: var(--lane-main)">AI는 속도를 주고, 판단과 책임은 기여자에게 남습니다.</span></div>

<!--
AI만으로 만든 PR을 검증 없이 던지는 것은 오픈소스 문화를 해칩니다. 메인테이너가 자발적으로 내는 시간을 낭비하기 때문입니다.

그래서 AI가 코드를 제안해도 PR을 올리기 전에 방향과 정확성은 내가 직접 확인합니다. 최소한의 변경으로 좁히고 테스트를 포함하고, 무엇을 왜 바꿨는지 설명을 답니다.

AI는 속도를 줍니다. 하지만 그 PR에 대한 판단과 책임은 기여자인 나에게 남습니다.
-->
...
- 좋은 이슈를 **찾고**(라벨·재현법·활발함), **AI로 분석**해 방향을 잡는다
- 규칙을 읽고 **먼저 말을 건 뒤** 코드를 짠다
- worktree + gh로 **이미 배운 루프**를 그대로 돌린다
- PR은 **검증하고 최소로** — 오픈소스 문화를 존중한다
...
<div class="pt-6 text-xl opacity-75 text-center">
이슈 하나 → 검증된 첫 PR. 남는 건 <span style="color: var(--lane-main)">세상의 코드에 남긴 내 커밋</span>입니다.
</div>

<!--
오픈소스 기여의 전체 흐름을 한 장으로 정리합니다. 좋은 이슈를 찾고, AI로 분석하고, 규칙을 읽고, 먼저 말을 걸고, 이미 배운 루프를 돌리고, PR을 검증해서 올립니다.

이 과정을 한 번 통과하면 여러분의 커밋이 세상의 코드에 남습니다.

다음에는 이렇게 남긴 기여와 저장소를 포트폴리오로 다듬고, GitHub 프로필을 완성하는 법을 보겠습니다.
-->
